    ...
    >
      ...
      >
        <p class="pb-8">
          ...
          hajlékony, mégis tartós szerkezetet adnak. Kényelmes viselet, időtálló
          forma és valódi kézműves minőség minden párban.
        </p>
      </div>
    </section>
    <div
      class="flex flex-wrap gap-6 p-6 bg-[var(--primary-brown-0)] justify-center"
    >
      <ProductCard
...
const products = Array.from({ length: 24 }, (_, i) => {
  ...
  const sizes = Array.from({ length: getRandomInt(3, 6) }, () => getRandomInt(39, 45)); // 39-45 méretek
  const sku = `SKU${i + 1}${getRandomInt(10, 99)}`;
  return {
    name: `Cipő ${i + 1}`,
    image: randomImage,
    price,
